...
GENERE            = os.environ.get('GENERE', 'electronic')   # 'hard' o 'electronic'
SETMANA_TEXT      = os.environ.get('SETMANA_TEXT', '').strip()
FONS_URL          = os.environ.get('FONS_URL', '').strip()    # link general de reserva (nomes visual)
MARGE_FONS        = 15.0
PADDING_FONS      = 1.0
...
print(f"\nTracks ({len(tracks)}):")
for t in tracks:
    print(f"  #{t['pos']}: {t['nom']} - {t['artista']}" + (" [imatge del link general]" if t.get('visual_general') else ""))
...
if GENERE == 'hard':
    ...
    titol_l1 = "TOP 5 EDM"
titol_l2 = "RELEASES THIS WEEK"
subtitol_setmana = SETMANA_TEXT if SETMANA_TEXT else "NEW THIS WEEK"

offsets_fons = {}
if FONS_URL:
    print(f"\nLink general de reserva (nomes visual): {FONS_URL}")
    ...
    if durada_fons_total:
        ...
        print(f"   AVIS: no s'ha pogut consultar la durada, es descarta el link general")
        FONS_URL = ''
...
for track in tracks:
    ...
    # ---------- CANCO (audio SEMPRE d'aqui, i visual per defecte tambe) ----------
    if yt_url:
    ...
    if ret != 0 or not os.path.exists(video_path) or os.path.getsize(video_path) < 10000:
        print(f"   No s'ha trobat cap font d'audio - usant portada")
    ...
    # ---------- VISUAL: per defecte el mateix video de la canco; si es marca, tram del link general ----------
    ...
    if visual_general and FONS_URL and pos in offsets_fons:
        ...
        print(f"   Imatge del link general seleccionada ({int(offset_fons//60):02d}:{int(offset_fons%60):02d})")
        ok_fons = baixar_tros_fons(FONS_URL, inici_baixada, durada_baixada, fons_path)
        if ok_fons:
            usar_fons_visual = True
        else:
            print(f"   AVIS: no s'ha pogut baixar el tram del link general, s'usa la imatge propia")
    ...
    txt.append(f"drawtext=fontfile='{FONT_SEMIBOLD}':text='{subtitol_setmana}':fontsize=32:fontcolor={COLOR_ACCENT}:borderw=2:bordercolor=black@0.6:x=(w-text_w)/2:y={Y_TITOL2}")
...
output_final = f"{OUTPUT}/top5releases_final.mp4"
